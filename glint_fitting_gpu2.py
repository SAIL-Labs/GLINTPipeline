...
def MCfunction(bins0, na, mu_opd, sig_opd):
    ...
    global fichier, start_iter, stop_iter
...
#    bins = cp.asarray(bins, dtype=cp.float32)
    count += 1
    stop_iter = time()
    print(int(count), na, mu_opd, sig_opd, stop_iter-start_iter)
    try:
        fichier.write('%s\t%s\t%s\t%s\t%s\n'%(int(count), na, mu_opd, sig_opd, stop_iter-start_iter))
    except:
        pass
...
#            bin_width = cp.mean(cp.diff(bins), dtype=cp.float32)
    ...
    if not mode_histo:
        ...
        accum = accum / nloop#cp.sum(accum, axis=-1, keepdims=True)
...
def basin_hoppin_values(mu_opd0, sig_opd0, na0, n_hop, bounds_mu, bounds_sig, bounds_na):
    ...
    for k in range(n_hop[0], n_hop[1]):
        for _ in range(1000):
            mu_opd = abs(np.random.normal(mu_opd0, 100))
        ...
        for _ in range(1000):
            sig_opd = abs(np.random.normal(sig_opd0, 100))
...
wl_range = np.arange(1600, 1610, 5)
for wl_rang in range(wl_range.size-1):
    print('///////', wl_range[wl_rang])
    ''' Settings '''  
    wl_min = wl_range[wl_rang]#1525
    wl_max = wl_range[wl_rang+1]#1575
    ...
    mu_opd0 = np.ones(6,)*260
    sig_opd0 = np.ones(6,)*150 # In nm
    na0 = np.ones(6,)*0.
    dphase_bias = 0.
    ...
    #        count += 1
        ...
        for basin_hopping_count in range(basin_hopping_nloop[0], basin_hopping_nloop[1]):
            sys.stdout = Logger(save_path+'basin_hop_%02d'%(basin_hopping_count)+'_%s.log'%wl_range[wl_rang])
            print('-------------')
            ...
            if not chi2_map_switch:
                if skip_fit:    
                    ...
                    popt = np.array([[na, mu_opd, sig_opd]])
    ...
    if mode_histo:
        pickle_name = os.path.basename(file_path[:-1])+'_pdf_%s.pkl'%wl_range[wl_rang]
    else:
        pickle_name = os.path.basename(file_path[:-1])+'_cdf_%s.pkl'%wl_range[wl_rang]
